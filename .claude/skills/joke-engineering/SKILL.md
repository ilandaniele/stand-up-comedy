...
## Localización rioplatense (Arg/Uy)

Cuando el material está en español rioplatense, aplicá el mismo framework pero teniendo en cuenta:

- **Voz primero (State H5).** El registro objetivo es ácido y absurdo, en voseo. Al transformar un chiste, nunca lo neutralices a "español de manual": mantené modismos, puteadas medidas y el fraseo del comediante. Un remate técnicamente perfecto que no suena a la persona es un fallo, no un éxito.
- **Connection Distance y referencias (H2 / Connection Resilience).** Las referencias muy locales (políticos, marcas, barrios, programas de TV) suben la autenticidad pero bajan la resiliencia. Está bien usarlas si el público es local; marcá cuándo un chiste depende de una referencia que no viaja.
- **Compresión (H6) en español.** El español tiende a ser más largo que el inglés. Vigilá el ratio conexión/palabra: cortá conectores, muletillas ("o sea", "viste", "nada") y subordinadas que diluyen el remate.
- **Absurdo y densidad (H3).** Para el registro absurdo, la "Nested Absurdity" (capas de creciente imposibilidad) y el "Irony Gradient" (mucho esfuerzo → poca importancia) son las mejores herramientas. Escalá la premisa hasta que se rompa la lógica; no la expliques.
